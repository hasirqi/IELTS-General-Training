...
function WordLab({state,update,done}:{state:LearningState;update:UpdateState;done:()=>void}) {
  ...
  if (!ids.length || state.wordIndex >= ids.length) return <section className="lesson-content"><LearningEmpty title="本轮新词已经完成" body="先让记忆稳定；下一轮会继续沿着1,000条学习顺序前进。" action="进入句型" onAction={done}/></section>;
  ...
  const learn = (confidence: number) => {
    update((current) => {
      ...
      const progress = introduceLexiconItem(current.lexiconProgress[item.id], item.id, { confidence, supportsUse:item.contentStatus === "verified" });
  ...
  return <section className="lesson-content word-lab"><div className="lesson-kicker">新词 {index+1} / {ids.length} · 词库总进度 {state.nextLexiconIndex}/1,000</div><div className="word-heading"><div><div className={`content-status ${item.contentStatus}`}>{item.contentStatus === "verified" ? "形·义·用已审核" : "当前练形·义"}</div><h1>{item.term}</h1><p>{item.phonetic || "发音标记待补"} · {item.part}</p></div><button className="round-button" onClick={() => speakEnglish(item.term)} aria-label={`播放 ${item.term} 的英语发音`}><IconVolume/></button></div><div className="meaning-block"><span>核心概念</span><strong>{item.meaning}</strong>{item.example && <p>{item.example}</p>}</div>{item.contentStatus === "verified" ? <div className="word-grid"><div><span>常见搭配</span><strong>{item.collocation}</strong></div><div><span>使用场景</span><strong>{item.category}</strong></div></div> : <div className="content-gate"><IconAlertTriangle/><div><strong>用法内容尚未通过人工审核</strong><p>本轮只练词形和核心义，不用模板例句冒充真实语境。</p></div></div>}<div className="recall-check"><p>先遮住解释：你能独立说出核心意思吗？</p>{!shown ? <button className="outline-button" onClick={() => setShown(true)}>想过了，评估自己</button> : <div className="confidence-row"><button onClick={() => learn(2)}>还不清楚</button><button onClick={() => learn(3)}>理解词义</button><button onClick={() => learn(5)} disabled={item.contentStatus !== "verified"}>能在句中使用</button></div>}</div></section>;
}
...
function Library() {
  ...
  return <section className="lesson-content library-page"><div className="lesson-kicker">全部 1,000 条均进入学习顺序 · 用法内容分批人工验收</div><h1>离线词库</h1><div className="library-quality"><div><strong>{quality.verified}</strong><span>条形·义·用已审核</span></div><div><strong>{quality.coreOnly}</strong><span>条当前只练形·义</span></div></div><div className="library-tools expanded"><label className="search-field"><span className="sr-only">搜索词库</span><IconSearch/><input aria-label="搜索英文或中文" value={query} onChange={(event) => resetPage(setQuery,event.target.value)} placeholder="搜英文或中文"/></label><label><span>类型</span><select aria-label="按词条类型筛选" value={kind} onChange={(event) => resetPage(setKind,event.target.value)}><option value="all">全部类型</option><option value="chunk">词块 120</option><option value="word">单词 880</option></select></label><label><span>内容状态</span><select aria-label="按内容审核状态筛选" value={status} onChange={(event) => resetPage(setStatus,event.target.value)}><option value="all">全部状态</option><option value="verified">形·义·用已审核</option><option value="core-only">当前只练形·义</option></select></label></div><p className="result-count">找到 {filtered.length} 条 · 第 {current}/{pages} 页 · 本页 {list.length} 条</p><div className="lexicon-grid">{list.map((item,index) => <article key={item.id}><div><em>#{(current-1)*PAGE_SIZE+index+1} · {item.kind==="chunk"?"词块":"单词"}</em><button onClick={() => speakEnglish(item.term)} aria-label={`播放 ${item.term} 的英语发音`} title={`播放 ${item.term}`}><IconVolume/></button></div><div className={`mini-status ${item.contentStatus}`}>{item.contentStatus === "verified" ? "用法已审核" : "核心义"}</div><h2>{item.term}</h2><small>{item.phonetic}</small><p>{item.meaning}</p>{item.example && <blockquote>{item.example}</blockquote>}</article>)}</div><nav className="library-pagination" aria-label="词库分页"><button disabled={current===1} onClick={() => go(current-1)}><IconArrowLeft/>上一页</button><span>第 <strong>{current}</strong> 页，共 {pages} 页</span><button disabled={current===pages} onClick={() => go(current+1)}>下一页<IconArrowRight/></button></nav></section>;
}

function Progress({state}:{state:LearningState}) {
  const stats = masteryStats(state.lexiconProgress);
  const accuracy = state.attempts ? Math.round(state.correct/state.attempts*100) : 0;
  const week = Math.min(36,Math.max(1,Math.floor(state.nextLexiconIndex/(1000/36))+1));
  const roadmap = roadmapWeeks[week-1];
  const total = curriculum.length;
  return <section className="lesson-content progress-page"><div className="lesson-kicker">能力地图 · 只使用真实训练数据</div><h1>当前学习进度</h1><div className="roadmap-current"><span>36 周路线 · 第 {week} 周</span><strong>{roadmap.theme}</strong><p>{roadmap.stage} · {roadmap.skillFocus}</p></div><div className="skill-list">{Object.entries(state.skill).map(([name,value]) => <div key={name}><span>{({listening:"听力",reading:"阅读",writing:"写作",speaking:"口语"} as Record<string,string>)[name]}</span><progress max="6" value={value}/><strong>{value.toFixed(1)}</strong></div>)}</div><div className="metrics expanded"><div><span>词库已接触</span><strong>{stats.introduced}/1000</strong></div><div><span>综合掌握</span><strong>{stats.average}%</strong></div><div><span>回忆正确率</span><strong>{accuracy}%</strong></div><div><span>课程完成</span><strong>{state.completedLessons.length}/{total}</strong></div></div></section>;
}
