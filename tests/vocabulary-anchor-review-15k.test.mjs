...
test("15K authored batch contains 5 manually reviewed sense-matched anchors", () => {
  const authored15K = reviewed.filter((anchor) => anchor.frequencyBand === "15K");
  assert.equal(authored15K.length, 5);
  const currentBatch = reviewed.filter((anchor) => {
    const id = Number(anchor.id.slice(7));
    return id >= 571 && id <= 575;
  });
  assert.equal(currentBatch.length, 5);
  assert.deepEqual(
    currentBatch.map((anchor) => anchor.term).sort(),
    Object.keys(vocabularyAnchorReview15K).sort(),
  );

  for (const anchor of currentBatch) {
    const authored = vocabularyAnchorReview15K[anchor.term];
...
test("15K review narrows high-band words to the sense used by their context", () => {
  const expectedSenses = {
    enrich: ["verb", "丰富；改善"],
    hasty: ["adjective", "仓促的"],
    irreversible: ["adjective", "不可逆转的"],
    monologue: ["noun", "独白；长篇独谈"],
    morbid: ["adjective", "病态的；与死亡有关的"],
  };
