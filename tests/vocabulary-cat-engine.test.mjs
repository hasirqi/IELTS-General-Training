...
const anchors = JSON.parse(fs.readFileSync(new URL("../src/content/vocabulary-anchor-bank-525.json", import.meta.url), "utf8"));
...
test("525 reviewed anchors extend M3 into the 11K band", () => {
  assert.equal(anchors.length,525); assert.equal(new Set(anchors.map((item)=>item.id)).size,525);
  assert.equal(new Set(anchors.map((item)=>item.familyId)).size,525);
  ...
  assert.equal(eligibleVocabularyAnchors(anchors).length,525);
});
