...
const anchors = JSON.parse(fs.readFileSync(new URL("../src/content/vocabulary-anchor-bank-590.json", import.meta.url), "utf8"));
...
test("590 reviewed anchors extend M3 into the 18K upper band", () => {
  assert.equal(anchors.length,590); assert.equal(new Set(anchors.map((item)=>item.id)).size,590);
  assert.equal(new Set(anchors.map((item)=>item.familyId)).size,590);
  ...
  assert.equal(eligibleVocabularyAnchors(anchors).length,590);
});
