...
import{verifiedCoreWordBatch13 as batch}from"../src/content/verified-core-word-batch-13.mjs";

const read=path=>fs.readFileSync(new URL(path,import.meta.url),"utf8");
const json=path=>JSON.parse(read(path));
const raw=json("../src/content/lexicon.json");
const candidates=json("../src/content/core-word-batch-13-candidates.json");
const course=read("../src/content/curriculum-batch-20.ts");
const foundation=read("../src/content/foundation-batch-16.ts");
...
test("batch 13 follows source priority 2401-2600",()=>{
  assert.equal(candidates.length,200);
  assert.deepEqual(candidates.map(item=>item.priorityIndex),Array.from({length:200},(_,index)=>2401+index));
  assert.deepEqual(Object.keys(batch),candidates.map(item=>item.term));
  assert.equal(raw.length,3600);
  assert.equal(new Set(raw.map(item=>item.term.toLowerCase())).size,3600);
  assert.deepEqual(raw.slice(3400).map(item=>item.term),candidates.map(item=>item.term));
});

test("batch 13 word content passes all gates",()=>{
  ...
  assert.deepEqual(lexiconQuality(built),{total:3600,verified:3600,coreOnly:0});
  const prior=buildLearningLexicon(raw.slice(0,3400));
...
test("batch 20 adds complete IELTS lessons",()=>{
  for(const prefix of["l","r","w","s"])assert.deepEqual(ids(course,prefix),Array.from({length:4},(_,index)=>`${prefix}${113+index}`));
...
test("batch 20 objective questions are answerable",()=>{
...
test("batch 20 fixed MP3 files are valid",()=>{
  for(let index=113;index<=116;index+=1){
...
test("foundation batch 16 adds ids 161-168",()=>{
  assert.deepEqual(ids(foundation,"g"),Array.from({length:8},(_,index)=>`g${161+index}`));
  assert.deepEqual(ids(foundation,"d"),Array.from({length:8},(_,index)=>`d${161+index}`));
