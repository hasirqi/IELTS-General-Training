...
test("ships exactly 1,000 unique lexical items", () => {
  assert.equal(lexicon.length, 1000);
  assert.equal(new Set(lexicon.map((item) => item.term.toLowerCase())).size, 1000);
  assert.equal(lexicon.filter((item) => item.kind === "chunk").length, 120);
  assert.equal(lexicon.filter((item) => item.kind === "word").length, 880);
});
