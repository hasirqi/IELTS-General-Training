...
test("ships exactly 2,000 unique lexical items", () => {
  assert.equal(lexicon.length, 2000);
  assert.equal(new Set(lexicon.map((item) => item.term.toLowerCase())).size, 2000);
  assert.equal(lexicon.filter((item) => item.kind === "chunk").length, 120);
  assert.equal(lexicon.filter((item) => item.kind === "word").length, 1880);
});
