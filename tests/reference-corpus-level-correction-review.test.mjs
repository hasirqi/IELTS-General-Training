...
test("correction review packets cover the same 70 texts in different orders", () => {
  assert.equal(manifests.packets.length, 2);
  const [first, second] = manifests.packets;
  assert.equal(first.itemCount, 70);
  assert.equal(second.itemCount, 70);
  assert.deepEqual([...first.order].sort(), [...second.order].sort());
  assert.notDeepEqual(first.order, second.order);
  assert.equal(new Set(first.order).size, 70);
});

test("materialized packets expose only text, title, rubric and empty response", () => {
