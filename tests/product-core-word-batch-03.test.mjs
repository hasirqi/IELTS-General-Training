...
const candidates=json("../src/content/core-word-batch-03-candidates.json");
const baselineRaw=json("../src/content/lexicon-baseline-1000.json");
const course=read("../src/content/curriculum-batch-10.ts");
const foundation=read("../src/content/foundation-batch-06.ts");
...
test("batch 03 follows exact source priority 401-600 and publishes only reviewed rows",()=>{
  assert.equal(candidates.length,200);
  assert.deepEqual(candidates.map((item)=>item.priorityIndex),Array.from({length:200},(_,index)=>index+401));
  assert.ok(candidates.every((item)=>item.batch===3&&item.sourceStatus==="ready"));
  assert.equal(new Set(candidates.map((item)=>item.term)).size,200);
  assert.deepEqual(Object.keys(verifiedCoreWordBatch03),candidates.map((item)=>item.term));
  assert.equal(raw.length,1600);
  assert.equal(new Set(raw.map((item)=>item.term.toLowerCase())).size,1600);
  assert.deepEqual(raw.slice(1400).map((item)=>item.term),candidates.map((item)=>item.term));
  assert.deepEqual(raw.slice(1400).map((item)=>item.id),Array.from({length:200},(_,index)=>`word-${String(index+1401).padStart(4,"0")}`));
});

test("all batch 03 meanings, contexts and collocations pass every earlier-batch gate",()=>{
  const baseline=buildBaselineLexicon(baselineRaw);
  const previous=[...Object.values(verifiedCoreWordBatch01),...Object.values(verifiedCoreWordBatch02)];
  ...
  for(const [term,item] of Object.entries(verifiedCoreWordBatch03)){
  ...
  assert.deepEqual(lexiconQuality(buildLearningLexicon(raw)),{total:1600,verified:1600,coreOnly:0});
});

test("batch 10 synchronously adds four complete lessons per IELTS skill",()=>{
  for(const prefix of ["l","r","w","s"]) assert.deepEqual(ids(course,prefix),Array.from({length:4},(_,index)=>`${prefix}${index+73}`));
...
test("all batch 10 objective questions are English, unique and answerable",()=>{
...
test("batch 10 listening uses four valid fixed MP3 files",()=>{
  for(let index=73;index<=76;index+=1){const bytes=fs.readFileSync(new URL(`../public/audio/l${index}.mp3`,import.meta.url));assert.ok(bytes.length>100_000);assert.ok(bytes.subarray(0,3).toString("ascii")==="ID3"||bytes[0]===0xff);}
});

test("foundation batch 06 adds exact ids 81-88 with unique usable tasks",()=>{
  assert.deepEqual(ids(foundation,"g"),Array.from({length:8},(_,index)=>`g${index+81}`));
  assert.deepEqual(ids(foundation,"d"),Array.from({length:8},(_,index)=>`d${index+81}`));
