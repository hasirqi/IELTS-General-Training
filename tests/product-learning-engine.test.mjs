...
test("verified usage content is explicit and never fabricated for unreviewed words", () => {
  ...
  assert.equal(quality.verified, 282);
  assert.equal(quality.coreOnly, 718);
...
test("overall progress is derived from real coverage, mastery and lessons", () => {
  ...
  const early = overallProgress({ introduced:stats.introduced,totalVocabulary:1000,completedLessons:1,totalLessons:96,masteryAverage:stats.average });
  const later = overallProgress({ introduced:700,totalVocabulary:1000,completedLessons:72,totalLessons:96,masteryAverage:72 });
