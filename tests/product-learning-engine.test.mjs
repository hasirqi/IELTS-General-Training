...
test("all 1,200 items enter one stable, unique study order", () => {
  const order = createStudyOrder(lexicon);
  assert.equal(order.length, 1200);
  assert.equal(new Set(order).size, 1200);
...
test("verified usage content is explicit and never fabricated for unreviewed words", () => {
  ...
  assert.equal(quality.total, 1200);
  assert.equal(quality.verified, 1200);
...
test("overall progress is derived from real coverage, mastery and lessons", () => {
  ...
  const early = overallProgress({ introduced:stats.introduced,totalVocabulary:1200,completedLessons:1,totalLessons:272,masteryAverage:stats.average });
  const later = overallProgress({ introduced:800,totalVocabulary:1200,completedLessons:204,totalLessons:272,masteryAverage:72 });
