...
test("verified usage content is explicit and never fabricated for unreviewed words", () => {
  ...
  assert.equal(quality.verified, 132);
  assert.equal(quality.coreOnly, 868);
...
test("overall progress is derived from real coverage, mastery and lessons", () => {
  ...
  const early = overallProgress({ introduced:stats.introduced,totalVocabulary:1000,completedLessons:1,totalLessons:64,masteryAverage:stats.average });
  const later = overallProgress({ introduced:700,totalVocabulary:1000,completedLessons:48,totalLessons:64,masteryAverage:72 });
