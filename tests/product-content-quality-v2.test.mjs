...
test("eighteen validated curriculum batches contain 432 unique lessons", () => {
  ...
  for (const group of groups) {
    assert.equal(group.length, 108);
    assert.equal(new Set(group).size, 108);
  }
  assert.equal(groups.flat().length, 432);
  assert.equal(new Set(groups.flat()).size, 432);
});

test("all 864 objective questions have English prompts and deterministic answers", () => {
  const listeningQuestions = questions(combined.listening);
  const readingQuestions = questions(combined.reading);
  assert.equal(listeningQuestions.length, 432);
  assert.equal(readingQuestions.length, 432);
...
test("listening keeps all four official parts and fixed audio coverage", () => {
  const expected = { "Part 1": 28, "Part 2": 28, "Part 3": 26, "Part 4": 26 };
  ...
  assert.equal(new Set(files).size, 108);
});

test("reading, writing and speaking retain complete official structures", () => {
  const expected = [
    [combined.reading, "Section 1", 35], [combined.reading, "Section 2", 45], [combined.reading, "Section 3", 28],
    [combined.writing, "Task 1", 54], [combined.writing, "Task 2", 54],
    [combined.speaking, "Part 1", 35], [combined.speaking, "Part 2", 34], [combined.speaking, "Part 3", 39],
  ];
...
test("foundation banks and the 36-week roadmap remain substantive and unique", () => {
  ...
  assert.equal(sentenceIds.length, 152);
  assert.equal(new Set(sentenceIds).size, 152);
  assert.equal(drillIds.length, 152);
  assert.equal(new Set(drillIds).size, 152);
  assert.deepEqual(weeks, Array.from({ length: 36 }, (_, index) => index + 1));
});

test("all published course sources contain no placeholder copy", () => {
  for (const source of [listening, reading, writing, speaking, batch02, batch03, batch04, batch05, batch06, batch07, batch08, batch09, batch10, batch11, batch12, batch13, batch14, batch15, batch16, batch17, batch18, sentence, drills, roadmap]) {
