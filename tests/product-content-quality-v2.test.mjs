...
test("twelve validated curriculum batches contain 336 unique lessons", () => {
  ...
  for (const group of groups) {
    assert.equal(group.length, 84);
    assert.equal(new Set(group).size, 84);
  }
  assert.equal(groups.flat().length, 336);
  assert.equal(new Set(groups.flat()).size, 336);
});

test("all 672 objective questions have English prompts and deterministic answers", () => {
  const listeningQuestions = questions(combined.listening);
  const readingQuestions = questions(combined.reading);
  assert.equal(listeningQuestions.length, 336);
  assert.equal(readingQuestions.length, 336);
...
test("listening keeps all four official parts and fixed audio coverage", () => {
  const expected = { "Part 1": 22, "Part 2": 22, "Part 3": 20, "Part 4": 20 };
  ...
  assert.equal(new Set(files).size, 84);
});

test("reading, writing and speaking retain complete official structures", () => {
  const expected = [
    [combined.reading, "Section 1", 29], [combined.reading, "Section 2", 33], [combined.reading, "Section 3", 22],
    [combined.writing, "Task 1", 42], [combined.writing, "Task 2", 42],
    [combined.speaking, "Part 1", 29], [combined.speaking, "Part 2", 28], [combined.speaking, "Part 3", 27],
  ];
...
test("foundation banks and the 36-week roadmap remain substantive and unique", () => {
  ...
  assert.equal(sentenceIds.length, 104);
  assert.equal(new Set(sentenceIds).size, 104);
  assert.equal(drillIds.length, 104);
  assert.equal(new Set(drillIds).size, 104);
  assert.deepEqual(weeks, Array.from({ length: 36 }, (_, index) => index + 1));
});

test("all published course sources contain no placeholder copy", () => {
  for (const source of [listening, reading, writing, speaking, batch02, batch03, batch04, batch05, batch06, batch07, batch08, batch09, batch10, batch11, batch12, sentence, drills, roadmap]) {
