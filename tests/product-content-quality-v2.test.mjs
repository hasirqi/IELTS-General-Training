...
const courseSources = [listening, reading, writing, speaking, batch02, batch03, batch04, batch05, batch06, batch07, batch08, batch09, batch10, batch11, batch12, batch13, batch14, batch15, batch16, batch17, batch18, batch19];
...
test("nineteen validated curriculum batches contain 448 unique lessons", () => {
  ...
  for (const group of groups) {
    assert.equal(group.length, 112);
    assert.equal(new Set(group).size, 112);
  }
  assert.equal(groups.flat().length, 448);
  assert.equal(new Set(groups.flat()).size, 448);
  const titles = courseSources.flatMap((source) => [...source.matchAll(/title:\s*"([^"]+)"/g)].map((match) => match[1]));
  assert.equal(titles.length, 448);
  const newTitles = [...batch19.matchAll(/title:\s*"([^"]+)"/g)].map((match) => match[1]);
...
test("all 896 objective questions have English prompts and deterministic answers", () => {
  const listeningQuestions = questions(combined.listening);
  const readingQuestions = questions(combined.reading);
  assert.equal(listeningQuestions.length, 448);
  assert.equal(readingQuestions.length, 448);
  const allPrompts = [...listeningQuestions, ...readingQuestions].map((question) => question.prompt);
  const newPrompts = questions(batch19).map((question) => question.prompt);
...
test("listening keeps all four official parts and fixed audio coverage", () => {
  const expected = { "Part 1": 29, "Part 2": 29, "Part 3": 27, "Part 4": 27 };
  ...
  assert.equal(new Set(files).size, 112);
});

test("reading, writing and speaking retain complete official structures", () => {
  const expected = [
    [combined.reading, "Section 1", 36], [combined.reading, "Section 2", 47], [combined.reading, "Section 3", 29],
    [combined.writing, "Task 1", 56], [combined.writing, "Task 2", 56],
    [combined.speaking, "Part 1", 36], [combined.speaking, "Part 2", 35], [combined.speaking, "Part 3", 41],
  ];
...
test("foundation banks and the 36-week roadmap remain substantive and unique", () => {
  ...
  assert.equal(sentenceIds.length, 160);
  assert.equal(new Set(sentenceIds).size, 160);
  assert.equal(drillIds.length, 160);
  assert.equal(new Set(drillIds).size, 160);
  assert.deepEqual(weeks, Array.from({ length: 36 }, (_, index) => index + 1));
});

test("all published course sources contain no placeholder copy", () => {
  for (const source of [listening, reading, writing, speaking, batch02, batch03, batch04, batch05, batch06, batch07, batch08, batch09, batch10, batch11, batch12, batch13, batch14, batch15, batch16, batch17, batch18, batch19, sentence, drills, roadmap]) {
