...
const batch21 = read("../src/content/curriculum-batch-21.ts");
const batch22 = read("../src/content/curriculum-batch-22.ts");
const batch23 = read("../src/content/curriculum-batch-23.ts");
...
const foundationBatch17 = read("../src/content/foundation-batch-17.ts");
const foundationBatch18 = read("../src/content/foundation-batch-18.ts");
const foundationBatch19 = read("../src/content/foundation-batch-19.ts");
...
const courseSources = [listening, reading, writing, speaking, batch02, batch03, batch04, batch05, batch06, batch07, batch08, batch09, batch10, batch11, batch12, batch13, batch14, batch15, batch16, batch17, batch18, batch19, batch20, batch21, batch22, batch23];
...
const batchSections21 = sectionsFor(batch21, "21");
const batchSections22 = sectionsFor(batch22, "22");
const batchSections23 = sectionsFor(batch23, "23");
const combined = {
...
${batchSections20.listening}\n${batchSections21.listening}\n${batchSections22.listening}\n${batchSections23.listening}`,
...
${batchSections20.reading}\n${batchSections21.reading}\n${batchSections22.reading}\n${batchSections23.reading}`,
...
${batchSections20.writing}\n${batchSections21.writing}\n${batchSections22.writing}\n${batchSections23.writing}`,
...
${batchSections20.speaking}\n${batchSections21.speaking}\n${batchSections22.speaking}\n${batchSections23.speaking}`,
};

test("twenty-three validated curriculum batches contain 512 unique lessons", () => {
  ...
  for (const group of groups) {
    assert.equal(group.length, 128);
    assert.equal(new Set(group).size, 128);
  }
  assert.equal(groups.flat().length, 512);
  assert.equal(new Set(groups.flat()).size, 512);
  const titles = courseSources.flatMap((source) => [...source.matchAll(/title:\s*"([^"]+)"/g)].map((match) => match[1]));
  assert.equal(titles.length, 512);
  const newTitles = [...batch23.matchAll(/title:\s*"([^"]+)"/g)].map((match) => match[1]);
...
test("all 1024 objective questions have English prompts and deterministic answers", () => {
  const listeningQuestions = questions(combined.listening);
  const readingQuestions = questions(combined.reading);
  assert.equal(listeningQuestions.length, 512);
  assert.equal(readingQuestions.length, 512);
  const allPrompts = [...listeningQuestions, ...readingQuestions].map((question) => question.prompt);
  const newPrompts = questions(batch23).map((question) => question.prompt);
...
test("listening keeps all four official parts and fixed audio coverage", () => {
  const expected = { "Part 1": 33, "Part 2": 33, "Part 3": 31, "Part 4": 31 };
  ...
  assert.equal(new Set(files).size, 128);
});

test("reading, writing and speaking retain complete official structures", () => {
  const expected = [
    [combined.reading, "Section 1", 40], [combined.reading, "Section 2", 55], [combined.reading, "Section 3", 33],
    [combined.writing, "Task 1", 64], [combined.writing, "Task 2", 64],
    [combined.speaking, "Part 1", 40], [combined.speaking, "Part 2", 39], [combined.speaking, "Part 3", 49],
  ];
...
test("foundation banks and the 36-week roadmap remain substantive and unique", () => {
  ...
  assert.equal(sentenceIds.length, 192);
  assert.equal(new Set(sentenceIds).size, 192);
  assert.equal(drillIds.length, 192);
  assert.equal(new Set(drillIds).size, 192);
  assert.deepEqual(weeks, Array.from({ length: 36 }, (_, index) => index + 1));
});

test("all published course sources contain no placeholder copy", () => {
  for (const source of [listening, reading, writing, speaking, batch02, batch03, batch04, batch05, batch06, batch07, batch08, batch09, batch10, batch11, batch12, batch13, batch14, batch15, batch16, batch17, batch18, batch19, batch20, batch21, batch22, batch23, sentence, drills, roadmap]) {
