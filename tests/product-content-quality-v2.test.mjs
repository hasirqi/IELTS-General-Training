...
test("sixteen validated curriculum batches contain 400 unique lessons", () => {
  ...
  for (const group of groups) {
    ...
    assert.equal(new Set(group).size, 100);
  }
  assert.equal(groups.flat().length, 400);
  assert.equal(new Set(groups.flat()).size, 400);
});

test("all 800 objective questions have English prompts and deterministic answers", () => {
  const listeningQuestions = questions(combined.listening);
  const readingQuestions = questions(combined.reading);
  assert.equal(listeningQuestions.length, 400);
  assert.equal(readingQuestions.length, 400);
...
test("listening keeps all four official parts and fixed audio coverage", () => {
  const expected = { "Part 1": 26, "Part 2": 26, "Part 3": 24, "Part 4": 24 };
...
test("reading, writing and speaking retain complete official structures", () => {
  const expected = [
    [combined.reading, "Section 1", 33], [combined.reading, "Section 2", 41], [combined.reading, "Section 3", 26],
    [combined.writing, "Task 1", 50], [combined.writing, "Task 2", 50],
    [combined.speaking, "Part 1", 33], [combined.speaking, "Part 2", 32], [combined.speaking, "Part 3", 35],
  ];
...
test("foundation banks and the 36-week roadmap remain substantive and unique", () => {
  ...
  assert.equal(sentenceIds.length, 136);
  assert.equal(new Set(sentenceIds).size, 136);
  assert.equal(drillIds.length, 136);
  assert.equal(new Set(drillIds).size, 136);
  assert.deepEqual(weeks, Array.from({ length: 36 }, (_, index) => index + 1));
});

test("all published course sources contain no placeholder copy", () => {
  for (const source of [listening, reading, writing, speaking, batch02, batch03, batch04, batch05, batch06, batch07, batch08, batch09, batch10, batch11, batch12, batch13, batch14, batch15, batch16, sentence, drills, roadmap]) {
