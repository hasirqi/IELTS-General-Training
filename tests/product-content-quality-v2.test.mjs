...
const courseSources = [listening, reading, writing, speaking, batch02, batch03, batch04, batch05, batch06, batch07, batch08, batch09, batch10, batch11, batch12, batch13, batch14, batch15, batch16, batch17, batch18, batch19, batch20, batch21, batch22];
...
test("twenty-two validated curriculum batches contain 496 unique lessons", () => {
  ...
  for (const group of groups) {
    assert.equal(group.length, 124);
    assert.equal(new Set(group).size, 124);
  }
  assert.equal(groups.flat().length, 496);
  assert.equal(new Set(groups.flat()).size, 496);
  const titles = courseSources.flatMap((source) => [...source.matchAll(/title:\s*"([^"]+)"/g)].map((match) => match[1]));
  assert.equal(titles.length, 496);
  const newTitles = [...batch22.matchAll(/title:\s*"([^"]+)"/g)].map((match) => match[1]);
...
test("all 992 objective questions have English prompts and deterministic answers", () => {
  const listeningQuestions = questions(combined.listening);
  const readingQuestions = questions(combined.reading);
  assert.equal(listeningQuestions.length, 496);
  assert.equal(readingQuestions.length, 496);
  const allPrompts = [...listeningQuestions, ...readingQuestions].map((question) => question.prompt);
  const newPrompts = questions(batch22).map((question) => question.prompt);
...
test("listening keeps all four official parts and fixed audio coverage", () => {
  const expected = { "Part 1": 32, "Part 2": 32, "Part 3": 30, "Part 4": 30 };
  ...
  assert.equal(new Set(files).size, 124);
});

test("reading, writing and speaking retain complete official structures", () => {
  const expected = [
    [combined.reading, "Section 1", 39], [combined.reading, "Section 2", 53], [combined.reading, "Section 3", 32],
    [combined.writing, "Task 1", 62], [combined.writing, "Task 2", 62],
    [combined.speaking, "Part 1", 39], [combined.speaking, "Part 2", 38], [combined.speaking, "Part 3", 47],
  ];
...
test("foundation banks and the 36-week roadmap remain substantive and unique", () => {
  ...
  assert.equal(sentenceIds.length, 184);
  assert.equal(new Set(sentenceIds).size, 184);
  assert.equal(drillIds.length, 184);
  assert.equal(new Set(drillIds).size, 184);
  assert.deepEqual(weeks, Array.from({ length: 36 }, (_, index) => index + 1));
});

test("all published course sources contain no placeholder copy", () => {
  for (const source of [listening, reading, writing, speaking, batch02, batch03, batch04, batch05, batch06, batch07, batch08, batch09, batch10, batch11, batch12, batch13, batch14, batch15, batch16, batch17, batch18, batch19, batch20, batch21, batch22, sentence, drills, roadmap]) {
