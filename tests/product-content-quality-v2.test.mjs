...
test("seven validated curriculum batches contain 256 unique lessons", () => {
  ...
  for (const group of groups) {
    assert.equal(group.length, 64);
    assert.equal(new Set(group).size, 64);
  }
  assert.equal(groups.flat().length, 256);
  assert.equal(new Set(groups.flat()).size, 256);
});

test("all 512 objective questions have English prompts and deterministic answers", () => {
  const listeningQuestions = questions(combined.listening);
  const readingQuestions = questions(combined.reading);
  assert.equal(listeningQuestions.length, 256);
  assert.equal(readingQuestions.length, 256);
...
test("listening keeps all four official parts and fixed audio coverage", () => {
  const expected = { "Part 1": 17, "Part 2": 17, "Part 3": 15, "Part 4": 15 };
...
test("reading, writing and speaking retain complete official structures", () => {
  const expected = [
    [combined.reading, "Section 1", 24], [combined.reading, "Section 2", 23], [combined.reading, "Section 3", 17],
    [combined.writing, "Task 1", 32], [combined.writing, "Task 2", 32],
    [combined.speaking, "Part 1", 24], [combined.speaking, "Part 2", 23], [combined.speaking, "Part 3", 17],
  ];
...
test("foundation banks and the 36-week roadmap remain substantive and unique", () => {
  ...
  assert.equal(sentenceIds.length, 64);
  assert.equal(new Set(sentenceIds).size, 64);
  assert.equal(drillIds.length, 64);
  assert.equal(new Set(drillIds).size, 64);
  assert.deepEqual(weeks, Array.from({ length: 36 }, (_, index) => index + 1));
});

test("all published course sources contain no placeholder copy", () => {
  for (const source of [listening, reading, writing, speaking, batch02, batch03, batch04, batch05, batch06, batch07, sentence, drills, roadmap]) {
