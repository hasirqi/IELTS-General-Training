...
import reviewed595 from "../src/content/vocabulary-anchor-bank-595.json" with { type: "json" };
...
const VERSION = "anchors-600-2026.07.28-v1";
const candidates = candidateManifest.candidates.filter((candidate) => candidate.frequencyBand === "20K");

if (candidates.length !== 5) throw new Error(`Expected 5 reviewed 20K candidates, received ${candidates.length}`);
if (Object.keys(vocabularyAnchorReview20K).length !== 5) throw new Error("The 20K review map must contain 5 entries");

const drafts = candidates.map((candidate) => {
  const authored = vocabularyAnchorReview20K[candidate.term.toLowerCase()];
...
const reviewed20K = drafts.map((draft, index) => ({
  ...
  difficulty: Number((-2.55 + ((draft.frequencyRank - 1) / 19_999) * 4.35).toFixed(3)),
  discrimination: 1,
  guessing: 0.25,
  reviewStatus: "item-authored",
  review: {
    reviewedAt: "2026-07-28",
    basis: "manual 20K POS and sense-focused English definition review over verified teaching context",
    checks: [
      "family",
      "frequency-band",
      "part-of-speech",
...
const anchors = [...reviewed595, ...reviewed20K];
