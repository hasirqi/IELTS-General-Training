...
import reviewed510 from "../src/content/vocabulary-anchor-bank-510.json" with { type: "json" };
...
const VERSION = "anchors-525-2026.07.28-v1";
const candidates = candidateManifest.candidates.filter((candidate) => candidate.frequencyBand === "11K");

if (candidates.length !== 15) throw new Error(`Expected 15 reviewed 11K candidates, received ${candidates.length}`);
if (Object.keys(vocabularyAnchorReview11K).length !== 15) throw new Error("The 11K review map must contain 15 entries");

const drafts = candidates.map((candidate) => {
  const authored = vocabularyAnchorReview11K[candidate.term.toLowerCase()];
...
const reviewed11K = drafts.map((draft, index) => ({
  ...
  difficulty: Number((-2.55 + ((draft.frequencyRank - 1) / 10_999) * 4.35).toFixed(3)),
  discrimination: 1,
  guessing: 0.25,
  reviewStatus: "item-authored",
  review: {
    reviewedAt: "2026-07-28",
    basis: "manual 11K POS and sense-focused English definition review over verified teaching context",
    checks: [
      "family",
      "frequency-band",
      "part-of-speech",
...
const anchors = [...reviewed510, ...reviewed11K];
