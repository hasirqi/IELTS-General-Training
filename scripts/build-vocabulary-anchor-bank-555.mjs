...
import reviewed540 from "../src/content/vocabulary-anchor-bank-540.json" with { type: "json" };
...
const VERSION = "anchors-555-2026.07.28-v1";
const candidates = candidateManifest.candidates.filter((candidate) => candidate.frequencyBand === "13K");

if (candidates.length !== 15) throw new Error(`Expected 15 reviewed 13K candidates, received ${candidates.length}`);
if (Object.keys(vocabularyAnchorReview13K).length !== 15) throw new Error("The 13K review map must contain 15 entries");

const drafts = candidates.map((candidate) => {
  const authored = vocabularyAnchorReview13K[candidate.term.toLowerCase()];
...
const reviewed13K = drafts.map((draft, index) => ({
  ...
  difficulty: Number((-2.55 + ((draft.frequencyRank - 1) / 12_999) * 4.35).toFixed(3)),
  discrimination: 1,
  guessing: 0.25,
  reviewStatus: "item-authored",
  review: {
    reviewedAt: "2026-07-28",
    basis: "manual 13K POS and sense-focused English definition review over verified teaching context",
    checks: [
      "family",
      "frequency-band",
      "part-of-speech",
...
const anchors = [...reviewed540, ...reviewed13K];
