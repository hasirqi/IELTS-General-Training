...
const auditDir = outputIndex >= 0 ? process.argv[outputIndex + 1] : path.resolve("audit", "level-correction-review");
const packet = (slot) => {
  const built = buildBlindedReviewPacket(corpus.records, slot);
  return {
    ...built,
    packetId: `level-correction-70-reviewer-${slot.toLowerCase()}-v1`,
...
const manifest = {
  version: "level-correction-review-packets-v1-2026.08.02",
  corpusVersion: corpus.version,
  status: "packet-manifests-only-no-labels",
  packets,
};

fs.writeFileSync(new URL("../src/content/reference-review-correction-packets-v1.json", import.meta.url), `${JSON.stringify(manifest, null, 2)}\n`);
...
console.log(`Built two isolated 70-item packets in ${auditDir}`);
