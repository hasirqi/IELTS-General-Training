...
import reviewed555 from "../src/content/vocabulary-anchor-bank-555.json" with { type: "json" };
...
const VERSION = "anchors-570-2026.07.28-v1";
const candidates = candidateManifest.candidates.filter((candidate) => candidate.frequencyBand === "14K");

if (candidates.length !== 15) throw new Error(`Expected 15 reviewed 14K candidates, received ${candidates.length}`);
if (Object.keys(vocabularyAnchorReview14K).length !== 15) throw new Error("The 14K review map must contain 15 entries");

const drafts = candidates.map((candidate) => {
  const authored = vocabularyAnchorReview14K[candidate.term.toLowerCase()];
...
const reviewed14K = drafts.map((draft, index) => ({
  ...
  difficulty: Number((-2.55 + ((draft.frequencyRank - 1) / 13_999) * 4.35).toFixed(3)),
  discrimination: 1,
  guessing: 0.25,
  reviewStatus: "item-authored",
  review: {
    reviewedAt: "2026-07-28",
    basis: "manual 14K POS and sense-focused English definition review over verified teaching context",
    checks: [
      "family",
      "frequency-band",
      "part-of-speech",
...
const anchors = [...reviewed555, ...reviewed14K];
