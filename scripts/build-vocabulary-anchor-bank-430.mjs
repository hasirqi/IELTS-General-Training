...
import reviewed380 from "../src/content/vocabulary-anchor-bank-380.json" with { type: "json" };
...
const VERSION = "anchors-430-2026.07.27-v1";
const candidates = candidateManifest.candidates.filter((candidate) => candidate.frequencyBand === "7K");

if (candidates.length !== 50) throw new Error(`Expected 50 reviewed 7K candidates, received ${candidates.length}`);
if (Object.keys(vocabularyAnchorReview7K).length !== 50) throw new Error("The 7K review map must contain 50 entries");

const drafts = candidates.map((candidate) => {
  const authored = vocabularyAnchorReview7K[candidate.term.toLowerCase()];
...
const reviewed7K = drafts.map((draft, index) => ({
  ...
  review: {
    ...
    basis: "manual 7K POS and sense-focused English definition review over verified teaching context",
    checks: [
      "family",
      "frequency-band",
      "part-of-speech",
...
const anchors = [...reviewed380, ...reviewed7K];
