...
import reviewed186 from "../src/content/vocabulary-anchor-bank-186.json" with { type: "json" };
...
const VERSION = "anchors-222-2026.07.27-v1";
const candidates = candidateManifest.candidates.filter((candidate) => candidate.frequencyBand === "2K");

if (candidates.length !== 36) throw new Error(`Expected 36 reviewed 2K candidates, received ${candidates.length}`);
if (Object.keys(vocabularyAnchorReview2K).length !== 36) throw new Error("The 2K review map must contain 36 entries");

const drafts = candidates.map((candidate) => {
  const authored = vocabularyAnchorReview2K[candidate.term.toLowerCase()];
...
const reviewed2K = drafts.map((draft, index) => ({
  ...
  review: {
    ...
    basis: "manual 2K POS and sense-focused English definition review over verified teaching context",
    checks: [
      "family",
      "frequency-band",
      "part-of-speech",
...
const anchors = [...reviewed186, ...reviewed2K];
