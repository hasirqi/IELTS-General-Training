...
import reviewed575 from "../src/content/vocabulary-anchor-bank-575.json" with { type: "json" };
...
const VERSION = "anchors-580-2026.07.28-v1";
const candidates = candidateManifest.candidates.filter((candidate) => candidate.frequencyBand === "16K");

if (candidates.length !== 5) throw new Error(`Expected 5 reviewed 16K candidates, received ${candidates.length}`);
if (Object.keys(vocabularyAnchorReview16K).length !== 5) throw new Error("The 16K review map must contain 5 entries");

const drafts = candidates.map((candidate) => {
  const authored = vocabularyAnchorReview16K[candidate.term.toLowerCase()];
...
const reviewed16K = drafts.map((draft, index) => ({
  ...
  difficulty: Number((-2.55 + ((draft.frequencyRank - 1) / 15_999) * 4.35).toFixed(3)),
  discrimination: 1,
  guessing: 0.25,
  reviewStatus: "item-authored",
  review: {
    reviewedAt: "2026-07-28",
    basis: "manual 16K POS and sense-focused English definition review over verified teaching context",
    checks: [
      "family",
      "frequency-band",
      "part-of-speech",
...
const anchors = [...reviewed575, ...reviewed16K];
