...
import reviewed525 from "../src/content/vocabulary-anchor-bank-525.json" with { type: "json" };
...
const VERSION = "anchors-540-2026.07.28-v1";
const candidates = candidateManifest.candidates.filter((candidate) => candidate.frequencyBand === "12K");

if (candidates.length !== 15) throw new Error(`Expected 15 reviewed 12K candidates, received ${candidates.length}`);
if (Object.keys(vocabularyAnchorReview12K).length !== 15) throw new Error("The 12K review map must contain 15 entries");

const drafts = candidates.map((candidate) => {
  const authored = vocabularyAnchorReview12K[candidate.term.toLowerCase()];
...
const reviewed12K = drafts.map((draft, index) => ({
  ...
  difficulty: Number((-2.55 + ((draft.frequencyRank - 1) / 11_999) * 4.35).toFixed(3)),
  discrimination: 1,
  guessing: 0.25,
  reviewStatus: "item-authored",
  review: {
    reviewedAt: "2026-07-28",
    basis: "manual 12K POS and sense-focused English definition review over verified teaching context",
    checks: [
      "family",
      "frequency-band",
      "part-of-speech",
...
const anchors = [...reviewed525, ...reviewed12K];
