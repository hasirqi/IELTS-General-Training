...
import reviewed330 from "../src/content/vocabulary-anchor-bank-330.json" with { type: "json" };
...
const VERSION = "anchors-380-2026.07.27-v1";
const candidates = candidateManifest.candidates.filter((candidate) => candidate.frequencyBand === "6K");

if (candidates.length !== 50) throw new Error(`Expected 50 reviewed 6K candidates, received ${candidates.length}`);
if (Object.keys(vocabularyAnchorReview6K).length !== 50) throw new Error("The 6K review map must contain 50 entries");

const drafts = candidates.map((candidate) => {
  const authored = vocabularyAnchorReview6K[candidate.term.toLowerCase()];
...
const reviewed6K = drafts.map((draft, index) => ({
  ...
  review: {
    ...
    basis: "manual 6K POS and sense-focused English definition review over verified teaching context",
    checks: [
      "family",
      "frequency-band",
      "part-of-speech",
...
const anchors = [...reviewed330, ...reviewed6K];
