...
import reviewed294 from "../src/content/vocabulary-anchor-bank-294.json" with { type: "json" };
...
const VERSION = "anchors-330-2026.07.27-v1";
const candidates = candidateManifest.candidates.filter((candidate) => candidate.frequencyBand === "5K");

if (candidates.length !== 36) throw new Error(`Expected 36 reviewed 5K candidates, received ${candidates.length}`);
if (Object.keys(vocabularyAnchorReview5K).length !== 36) throw new Error("The 5K review map must contain 36 entries");

const drafts = candidates.map((candidate) => {
  const authored = vocabularyAnchorReview5K[candidate.term.toLowerCase()];
...
const reviewed5K = drafts.map((draft, index) => ({
  ...
  review: {
    ...
    basis: "manual 5K POS and sense-focused English definition review over verified teaching context",
    checks: [
      "family",
      "frequency-band",
      "part-of-speech",
...
const anchors = [...reviewed294, ...reviewed5K];
