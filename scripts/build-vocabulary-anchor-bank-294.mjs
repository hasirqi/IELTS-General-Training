...
import reviewed258 from "../src/content/vocabulary-anchor-bank-258.json" with { type: "json" };
...
const VERSION = "anchors-294-2026.07.27-v1";
const candidates = candidateManifest.candidates.filter((candidate) => candidate.frequencyBand === "4K");

if (candidates.length !== 36) throw new Error(`Expected 36 reviewed 4K candidates, received ${candidates.length}`);
if (Object.keys(vocabularyAnchorReview4K).length !== 36) throw new Error("The 4K review map must contain 36 entries");

const drafts = candidates.map((candidate) => {
  const authored = vocabularyAnchorReview4K[candidate.term.toLowerCase()];
...
const reviewed4K = drafts.map((draft, index) => ({
  ...
  review: {
    ...
    basis: "manual 4K POS and sense-focused English definition review over verified teaching context",
    checks: [
      "family",
      "frequency-band",
      "part-of-speech",
...
const anchors = [...reviewed258, ...reviewed4K];
