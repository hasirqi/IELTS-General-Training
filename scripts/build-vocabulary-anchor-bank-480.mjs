...
import reviewed430 from "../src/content/vocabulary-anchor-bank-430.json" with { type: "json" };
...
const VERSION = "anchors-480-2026.07.28-v1";
const candidates = candidateManifest.candidates.filter((candidate) => candidate.frequencyBand === "8K");

if (candidates.length !== 50) throw new Error(`Expected 50 reviewed 8K candidates, received ${candidates.length}`);
if (Object.keys(vocabularyAnchorReview8K).length !== 50) throw new Error("The 8K review map must contain 50 entries");

const drafts = candidates.map((candidate) => {
  const authored = vocabularyAnchorReview8K[candidate.term.toLowerCase()];
...
const reviewed8K = drafts.map((draft, index) => ({
  ...
  review: {
    ...
    basis: "manual 8K POS and sense-focused English definition review over verified teaching context",
    checks: [
      "family",
      "frequency-band",
      "part-of-speech",
...
const anchors = [...reviewed430, ...reviewed8K];
