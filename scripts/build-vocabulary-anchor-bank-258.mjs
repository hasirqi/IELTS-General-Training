...
import reviewed222 from "../src/content/vocabulary-anchor-bank-222.json" with { type: "json" };
...
const VERSION = "anchors-258-2026.07.27-v1";
const candidates = candidateManifest.candidates.filter((candidate) => candidate.frequencyBand === "3K");

if (candidates.length !== 36) throw new Error(`Expected 36 reviewed 3K candidates, received ${candidates.length}`);
if (Object.keys(vocabularyAnchorReview3K).length !== 36) throw new Error("The 3K review map must contain 36 entries");

const drafts = candidates.map((candidate) => {
  const authored = vocabularyAnchorReview3K[candidate.term.toLowerCase()];
...
function optionSet(target, index, valueFor) {
  ...
  const values = pool.map(valueFor).filter((value, valueIndex, all) =>
...
const reviewed3K = drafts.map((draft, index) => ({
  ...
  definitionOptions: optionSet(draft, index, (entry) => entry.englishDefinition),
  correctDefinition: draft.englishDefinition,
  chineseOptions: optionSet(draft, index, (entry) => entry.focusedMeaning),
  ...
  review: {
    ...
    basis: "manual 3K POS and sense-focused English definition review over verified teaching context",
    checks: [
      "family",
      "frequency-band",
      "part-of-speech",
...
const anchors = [...reviewed222, ...reviewed3K];
